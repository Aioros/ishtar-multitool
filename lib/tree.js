  ...
  async search(options) {
    ...
    if (query) {
      ...
      if (prop == "name") {
        ...
        var language = (await chrome.storage.sync.get("language")).language || "en";

        relevantUrls = (await chrome.runtime.sendMessage({
          request: "searchPageContent",
          args: {query, language}
        })).map((page) => page.url);
      ...
      for (let i of nodes) {
        ...
        if (
          !relevantUrls.includes(i.dataset.url)
          && (
